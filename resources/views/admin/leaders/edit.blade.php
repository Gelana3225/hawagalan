    ...
    <form method="POST" action="{{ route('admin.leaders.update', $leader) }}" enctype="multipart/form-data">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea name="description" rows="5"
                      style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 0.9rem; resize: vertical;">{{ old('description', $leader->description) }}</textarea>
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            @else
            <img id="photo-preview" src="" alt="Preview" style="display: none; margin-bottom: 10px; width: 120px; height: 120px; object-fit: cover; border-radius: 10px;">
            @endif
            ...
            <p style="color: #9ca3af; font-size: 0.8rem; margin-top: 4px;">Leave empty to keep current photo.</p>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            ...
            <div>
                ...
                <select name="is_visible" style="width: 100%; padding: 10px 14px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 0.9rem;">
                    <option value="1" {{ old('is_visible', $leader->is_visible ? '1' : '0') == '1' ? 'selected' : '' }}>Yes</option>
                    <option value="0" {{ old('is_visible', $leader->is_visible ? '1' : '0') == '0' ? 'selected' : '' }}>No</option>
                </select>
            </div>
        </div>
